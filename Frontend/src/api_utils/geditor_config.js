...
const fetchPageContent = async (websiteId, pageId) => {
  try {
    ...
    if (response.data.status === 'new') {
      console.log("New Response Data ",response.data.content['mycustom-html'])
      return { htmlContent: response.data.content['mycustom-html'], cssContent: response.data.content['mycustom-css'], message: "new" };
    }
...
const fetchAssets = async () => {
  try {
    ...
    const imageData = response.data; // Array of image objects
...
const geditorConfig = async (assets, pageId) => {
  ...
  const dynamicAssets = await fetchAssets(); // Get image URLs

  console.log("Dynamic Assets ",dynamicAssets)
  ...
  const editorConfig = {
    ...
    assetManager: {
      assets: dynamicAssets, // ⬅️ Set fetched images here
    },
  ...
  console.log("BOOOOOOOOOOOOOOOOOOOOOOOOOO",htmlContent,cssContent)
